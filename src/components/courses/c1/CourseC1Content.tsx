export default function CourseC1Content({ t }: { t: (key: string) => string }) {
    return (
      <section className="mt-6 rounded-2xl border bg-white p-5 shadow-sm sm:p-6">
        <div className="text-sm text-gray-500">{t("descLabel")}</div>
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border">
          <table className="w-full min-w-[720px] text-sm">
            <thead className="bg-yellow-200/70 text-left">
              <tr>
                <th className="px-4 py-3 font-extrabold text-gray-900">
                  {t("table.col1")}
                </th>
                <th className="px-4 py-3 font-extrabold text-gray-900">
                  {t("table.col2")}
