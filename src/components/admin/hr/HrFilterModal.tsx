...
export default function HrFilterModal({
  open,
  title,
  columns,
  selected,
  ...
  open: boolean;
  title: string;
  columns: Array<{ key: HrFilterKey; label: string }>;
  selected: HrFilterKey[];
  onChange: (next: HrFilterKey[]) => void;
  onClose: () => void;
  onApply: () => void;
  onSelectAll: () => void;
  onClear: () => void;
}) {
  const t = useTranslations("HR.filter");
  ...
  function toggle(key: HrFilterKey) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-[640px] -translate-x-1/2 -translate-y-1/2 panel p-4">
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    active ? "border-pink-500 bg-pink-500" : "border-white/20"
                  )}
                />
              </button>
            );
